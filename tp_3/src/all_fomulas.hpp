...
// POWER
template <int N> class Puissance {public: static const long valeur(double v){return v * Puissance<N-1>::valeur(v);}};
...
// template <> class Exponentielle<1> {
//     public: static const double valeur(double v){return Puissance<1>::valeur(v);}
// };
...
template <int N> class Cosinus {
    public: static const long valeur(double v){return (Puissance<N>::valeur(-1) * (Puissance<2*N>::valeur(v) / (double)Factorielle<2*N>::valeur)) + Cosinus<N-1>::valeur(v);}
};
